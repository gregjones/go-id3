...
func TestCoverArt240(t *testing.T) {
	p := path.Join("..", "..", "test", "test_coverart240.mp3")
	t.Logf("With file %s", p)
	...
	t.Log(actual.Header.Version)
	if len(actual.Pictures) != 1 {
		t.Errorf("No pictures: expected 1, got %d", len(actual.Pictures))
	}
	coverImage := actual.GetCoverPicture()
	if coverImage == nil {
		t.Error("Cover Image is nil")
	}
